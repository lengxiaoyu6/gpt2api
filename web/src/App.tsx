...
} from "./components/AnnouncementCenter";
...
export default function App() {
  ...
  const handleTabChange = (tab: TabKey) => {
    if (
      (tab === "generate" || tab === "history" || tab === "profile") &&
      !user
    ) {
      openAuthForTab(tab);
      return;
  ...
  return (
    <AnnouncementProvider active={activeTab === "home"}>
      <div className="min-h-screen bg-background text-foreground transition-colors duration-300 font-sans selection:bg-primary/30 lg:flex lg:flex-row">
        ...
        <aside className="hidden border-r border-border/50 bg-card/55 backdrop-blur-xl lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-72 lg:flex-col lg:justify-between">
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive ? (
                      <span className="ml-auto h-2.5 w-2.5 rounded-full bg-white/95 shadow-sm" />
                    ) : null}
                  </button>
                );
              })}
          ...
          <div className="border-t border-border/50 px-5 py-6">
            {user ? (
              ...
              <Button
                onClick={() => openAuthForTab(activeTab)}
                className="h-12 w-full rounded-2xl gap-2 font-bold shadow-lg shadow-primary/20"
              >
        ...
        <div className="flex min-h-screen flex-1 flex-col">
            ...
            <div className="ml-2 flex shrink-0 items-center gap-1.5 sm:gap-2">
              ...
              {user ? (
                <div className="flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border border-border/50 bg-secondary/50 px-2.5 py-1.5 sm:gap-2 sm:px-3">
                  ...
                  <span className="whitespace-nowrap font-mono text-xs font-semibold sm:text-sm sm:font-medium">
                    {formatCredit(user.credit_balance)} 积分
                  </span>
                </div>
              ) : (
                <Button
              ...
              <div className="flex shrink-0 items-center gap-3 whitespace-nowrap">
                ...
                {user ? (
                  ...
                  <Button
                    variant="secondary"
                    onClick={() => openAuthForTab(activeTab)}
                    className="h-12 rounded-2xl gap-2 px-5 font-bold"
                  >
                    <LogIn className="h-4 w-4" />
                    登录账号
                  </Button>
          ...
          <main className="flex flex-1 flex-col pt-16 pb-[calc(4rem+env(safe-area-inset-bottom))] lg:px-6 lg:pt-0 lg:pb-10 xl:px-8">
            <motion.div
              ...
              className={cn(
                "w-full",
                !pageMotionEnabled && "mobile-page-panel-static",
                activeTab === "profile" && "flex flex-1 flex-col",
              )}
            >
              {activeTab === "home" && (
                <HomeView
                  siteName={siteName}
                  onStartGeneration={() => handleTabChange("generate")}
                  onOpenUpdateLogs={() => handleTabChange("updateLogs")}
                />
              )}
        ...
        <nav
          aria-label="移动底部导航"
          className="mobile-scroll-surface fixed bottom-0 left-0 right-0 z-40 border-t border-border/50 bg-background/95 pb-safe lg:hidden"
        >
        ...
        <div
          ...
          className="pointer-events-none fixed inset-0 -z-10 hidden overflow-hidden opacity-20 dark:opacity-40 lg:block"
        >
          <div className="absolute left-[-10%] top-[-10%] h-[50%] w-[50%] rounded-full bg-primary/30 blur-[120px] animate-pulse" />
          <div
            className="absolute bottom-[-10%] right-[-10%] h-[50%] w-[50%] rounded-full bg-blue-500/20 blur-[120px] animate-pulse"
