...
const checkinStatus = ref<meApi.MyCheckinStatus | null>(null);
const checkinLoading = ref(false);

async function loadAll() {
    loading.value = true;
    try {
        const [_, keys, models, s14, s1, logs, credits, checkin] = await Promise.all([
            store.fetchMe(),
            listKeys(1, 100),
            meApi.listMyModels(),
...
async function submitCheckin() {
    if (!checkinStatus.value?.enabled) {
        ElMessage.warning('签到奖励暂未开启');
        return;
    }
    if (checkinStatus.value.checked_in) {
        ElMessage.info('今日已签到');
        return;
    }
    checkinLoading.value = true;
    try {
        checkinStatus.value = await meApi.checkinToday();
        await loadAll();
        ElMessage.success(`签到成功，已领取 ${formatCredit(checkinStatus.value.awarded_credits)}`);
    } finally {
...
<template>
    <div class="page-container" v-loading="loading">
        ...
        <div class="hero-card">
            <div class="hero-main">
                ...
                <div class="hero-actions">
                    <el-button type="primary" :icon="Wallet" @click="go('/personal/billing')"> 充值积分 </el-button>
                    <el-button :icon="Key" @click="go('/personal/keys')"> 管理 API Key </el-button>
                    <el-button :icon="ChatLineRound" @click="go('/personal/play')"> 在线体验 </el-button>
        ...
        <div class="card-block checkin-card">
            <div class="checkin-main">
                <div>
                    <h3 class="page-title" style="margin: 0; font-size: 16px">每日签到</h3>
                    <div class="muted" style="margin-top: 4px">
                        <template v-if="checkinStatus?.enabled">
                            今日奖励 {{ formatCredit(checkinStatus?.today_reward_credits) }}
                            <span v-if="checkinStatus?.checked_at">
                                · 签到时间 {{ formatDateTime(checkinStatus?.checked_at) }}
                            </span>
                            <span v-else-if="checkinStatus?.last_checked_at">
                                · 上次签到 {{ formatDateTime(checkinStatus?.last_checked_at) }}
                            </span>
                        </template>
                        <template v-else> 当前未开启签到奖励 </template>
                    </div>
                </div>
                <div class="checkin-side">
                    <div v-if="checkinStatus?.enabled" class="checkin-reward">
                        +{{ formatCredit(checkinStatus?.today_reward_credits) }}
                    </div>
                    <el-tag
                        size="small"
                        :type="checkinStatus?.checked_in ? 'success' : (checkinStatus?.enabled ? 'warning' : 'info')"
                        effect="plain">
                        {{ checkinStatus?.checked_in ? '今日已签到' : (checkinStatus?.enabled ? '待签到' : '未开启') }}
                    </el-tag>
                    <el-button
                        type="success"
                        :loading="checkinLoading"
                        :disabled="!checkinStatus?.enabled || !!checkinStatus?.checked_in"
                        @click="submitCheckin">
                        {{ checkinStatus?.checked_in ? '今日已签到' : '立即签到' }}
                    </el-button>
                </div>
            </div>
        </div>
            ...
            <el-col :lg="16" :md="14" :sm="24">
                <div class="card-block">
                    <div class="flex-between" style="margin-bottom: 10px">
                        ...
                        <el-button text :icon="Promotion" @click="go('/personal/usage')"> 查看详情 </el-button>
                    </div>
...
.chart-tip .tip-dot.primary {
    background: var(--el-color-primary);
}
.chart-tip .tip-dot.danger {
