...
test('错误页与缺省返回入口改成后台首页', () => {
  const error403 = read('web/src/views/Error403.vue')
  const error404 = read('web/src/views/Error404.vue')
...
test('后台首页复用现有接口拼装数据概览与最近任务', () => {
  const dashboardVue = read('web/src/views/admin/Dashboard.vue')
  const adminApi = read('web/src/api/admin.ts')
...
test('个人中心页面源码已经从 web 端裁剪', () => {
  ...
  assert.equal(exists('web/src/views/personal/ApiKeys.vue'), false)
