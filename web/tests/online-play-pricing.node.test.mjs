...
test('在线体验页按模型能力控制张数和输出质量', () => {
  ...
  assert.match(playVue, /isLocalImagePool/)
  assert.match(playVue, /shouldSendOutputSize/)
  assert.match(playVue, /effectiveOutputQuality/)
  ...
  assert.match(playVue, /const isLocalImagePool = computed\(\(\) => currentImageModel\.value\?\.has_image_channel !== true\)/)
  assert.match(playVue, /const shouldSendOutputSize = computed\(\(\) => supportsOutputSize\.value \|\| isLocalImagePool\.value\)/)
  assert.match(playVue, /function effectiveOutputQuality\(quality: OutputQualityValue\): OutputQualityValue \{\s*return supportsOutputSize\.value \? quality : '1K'\s*\}/)
  assert.match(playVue, /\.\.\.\(shouldSendOutputSize\.value\s*\?\s*\{\s*size:\s*resolveOutputSize\(t2iRatio\.value,\s*effectiveOutputQuality\(t2iQuality\.value\)\)\s*\}\s*:\s*\{\s*\}\)/s)
  assert.match(playVue, /\.\.\.\(shouldSendOutputSize\.value\s*\?\s*\{\s*size:\s*resolveOutputSize\(i2iRatio\.value,\s*effectiveOutputQuality\(i2iQuality\.value\)\)\s*\}\s*:\s*\{\s*\}\)/s)
