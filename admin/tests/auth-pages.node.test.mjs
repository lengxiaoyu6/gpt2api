...
test('登录页改为常规后台登录页', () => {
  const loginVue = read('admin/src/views/auth/Login.vue')
...
test('注册页与公开营销布局源码已经从 web 端移除', () => {
  assert.equal(exists('admin/src/views/auth/Register.vue'), false)
  assert.equal(exists('admin/src/layouts/PublicLayout.vue'), false)
  assert.equal(exists('admin/src/views/public/Home.vue'), false)
  assert.equal(exists('admin/src/views/public/Showcase.vue'), false)
  assert.equal(exists('admin/src/views/public/Pricing.vue'), false)
})
