...
test('Prompt 库后端路由、装配与后台菜单已声明', () => {
  ...
  assert.match(routerGo, /admin\.Group\("\/prompt-categories",\s*middleware\.RequirePerm\(rbac\.PermSystemSetting\)\)/)
  assert.match(routerGo, /GET\("",\s*d\.PromptH\.ListAdminCategories\)/)
  assert.match(routerGo, /POST\("",\s*d\.PromptH\.CreateCategory\)/)
  assert.match(routerGo, /DELETE\("\/:id",\s*d\.PromptH\.DeleteCategory\)/)
...
test('admin 端声明 Prompt 库维护接口、路由与页面', () => {
  ...
  assert.match(apiTs, /PromptCategory/)
  assert.match(apiTs, /adminListPromptCategories/)
  assert.match(apiTs, /adminCreatePromptCategory/)
  assert.match(apiTs, /adminDeletePromptCategory/)
  assert.match(apiTs, /\/api\/admin\/prompt-categories/)
  ...
  assert.match(viewVue, /分类管理/)
  assert.match(viewVue, /关联 Prompt 将迁移到通用/)
  ...
  assert.match(viewVue, /el-select/)
  assert.match(viewVue, /adminListPromptCategories/)
})
