...
| `redis` | `addr`, `password`, `pool_size` | Redis,生产推荐 pool=500(锁 / 限流 / 令牌桶) |
...
Redis 在 Docker 部署中默认不发布宿主机端口,后端容器通过 `redis:6379` 访问。需要启用 Redis 密码时,在 `deploy/.env` 中设置 `REDIS_PASSWORD`,compose 会同时传给 Redis 与 server。
...
可以。修改 `.env` / `configs/config.yaml` 的 DSN 与 `redis.addr` 即可。Redis 建议至少 Redis 7,开启 AOF,并配置访问密码;MySQL 建议 8.0+,`max_connections >= 500`。
</details>
