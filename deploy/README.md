...
一条命令生成后端 + goose + 管理端 + Web 用户端四个产物:
...
```bash
ls -lh deploy/bin/gpt2api deploy/bin/goose admin/dist/index.html web/dist/index.html
```
...
如果前面还有一层 Nginx，推荐继续由 Nginx 终止 TLS，再把管理端与用户端域名都反代到同一个 `server:8080`。Go 服务会根据请求头 `Host` 在 `admin/dist` 与 `web/dist` 之间选择站点。

后台系统设置使用 `site.web_domain` 登记用户端域名。填入示例值 `img.domain.com` 后，访问 `admin.domain.com` 进入管理端，访问 `img.domain.com` 进入 Web 用户端。两端域名不同，浏览器登录态与本地存储也会自然分开。

> **⚠️ 没有默认账号 / 密码。** 启动完成后打开 Web 用户端的 `/register` 注册,
...
| 场景 | 做什么 |
|------|--------|
| 只改了前端 | `cd admin && npm run build`、`cd ../web && npm run build` → `cd ../deploy && docker compose build server && docker compose up -d server` |
